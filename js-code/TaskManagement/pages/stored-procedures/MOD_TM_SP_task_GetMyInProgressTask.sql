USE [awdb]
GO
/****** Object:  StoredProcedure [dbo].[MOD_TM_SP_task_GetMyInProgressTask]    Script Date: 1/7/2020 8:20:02 PM ******/
SET ANSI_NULLS ON
GO
...
-- =============================================
ALTER PROCEDURE [dbo].[MOD_TM_SP_task_GetMyInProgressTask]
  @PageNumber INT = 1,
  @PageSize   INT = 10,
  @Owner	  NVARCHAR(MAX),
  @sortBy	  varchar(20),
...
BEGIN
	...
	-- 
	where targetTaskPerformer.performerId= @Owner  and task.status > 0 and task.status < 10 and (taskPerformer.status <> 2)  and targetTaskPerformer.status <= 1 
	and task.isDeleted <> 1 
	and task.taskName like '%'+@TaskName+'%' and (task.progress = @Progress or @Progress =-1) and (task.startDate >= @startDate and task.dueDate <= @endDate) and (task.owner = @filterOwner or @filterOwner = -1)
	--task.startDate <=  CAST(GETDATE() AS DATE)
	order by 
