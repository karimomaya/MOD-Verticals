...
public class VacationsController {
    ...
    public ResponseBuilder<ObjectNode> getAll(
        ...
        try {
            ...
            endDate = cal.getTime();
            endDateString = Utils.convertDateToString(endDate, "yyyy-MM-dd");

            List<Vacation> resultVacations = new ArrayList<>();

            for(Vacation vacation : vacationList){
                Date fromDate = simpleDateFormat.parse(vacation.getFromDate());
                Date toDate = simpleDateFormat.parse(vacation.getToDate());
                if (Utils.isDateBetweenDates(fromDate,startDate,endDate)) {
                    if(Utils.isDateBetweenDates(toDate,startDate,endDate)){
                        resultVacations.add(vacation);
                    }else{
                        vacation.setToDate(simpleDateFormat.parse(endDateString));
                        resultVacations.add(vacation);
                    }
                }else if(Utils.isDateBetweenDates(toDate,startDate,endDate)){
                    vacation.setFromDate(simpleDateFormat.parse(startDateString));
                    resultVacations.add(vacation);
                }
            }
